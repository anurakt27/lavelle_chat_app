...
var app = {
  ...
  helpers: {
    ...
    updateRoomsList: function(room){
      var html = `<a href="/chat/${room._id}"><button>Join Room</button></a>`;

      if(html === ''){ return; }

      if($(".timeline").length % 2 == 1){
        $('.timeline').prepend("<div class='container right'><div class='content'><h2>"+room.title+"</h2>"+html+"</div></div>");
      }if($(".timeline").length % 2 == 0){
        $('.timeline').prepend("<div class='container left'><div class='content'><h2>"+room.title+"</h2>"+html+"</div></div>"); 
      }
